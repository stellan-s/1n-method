# The 1..n Method

A pragmatic approach to product development that optimizes for reality over activity.

## Overview

The 1..n Method is a specification for how product organizations make decisions about what to build and when. It prioritizes small, focused sets of work that replace existing reality rather than accumulating unbounded backlogs.

This repository contains the method specification—not an implementation or tool. Organizations can apply these principles using whatever tools fit their context.

**Method Specification Version:** 1.0.0

## Quick Start

- **New to the method?** Start with [Summary](docs/00-summary.md)
- **Understanding the foundation:** Read [Foundations](docs/01-foundations.md)
- **Core principles:** See [Method Principles](docs/02-method-principles.md)
- **How decisions flow:** Review [Decision Flow](docs/03-decision-flow.md)

## Documentation

1. [Summary](docs/00-summary.md) – One-page overview
2. [Foundations](docs/01-foundations.md) – Reality constraints that shape the method
3. [Method Principles](docs/02-method-principles.md) – Core operating principles
4. [Decision Flow](docs/03-decision-flow.md) – How work moves through the system
5. [What It Does Not Solve](docs/04-what-it-does-not-solve.md) – Explicit non-goals
6. [Leadership and "When?"](docs/05-leadership-and-when.md) – Answering timeline questions without false precision
7. [Metrics and Progress](docs/06-metrics-and-progress.md) – Measuring reality, not activity
8. [Glossary](docs/07-glossary.md) – Key terms and definitions

## Versioning

This repository follows semantic versioning for the method specification itself.

- **Major version** changes indicate fundamental shifts in method principles or foundations
- **Minor version** changes indicate additions or clarifications that don't alter core principles
- **Patch version** changes indicate corrections, typos, or formatting improvements

The current version is **1.0.0**.

## License

MIT License. See [LICENSE](LICENSE) for details.

## Contributing

This is v1.0 of the method specification. Feedback and discussion are welcome via issues. Proposed changes to the method itself will be evaluated carefully to maintain coherence and avoid scope creep.
